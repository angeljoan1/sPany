      ...
      {[0, 1, 2, 3, 4, 5].map(i => (
        <div
          ...
          className={`w-3.5 h-3.5 rounded-full transition-all duration-150 ${
            pin[i] ? 'bg-[#1A1A1A] scale-110' : 'bg-[rgba(26,26,26,0.15)]'
          }`}
        />
      ))}
    </div>
    ...
    <>
      <div className="grid grid-cols-3 gap-4 mb-4">
        {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
          <button
            key={num}
            onClick={() => { triggerHaptic(); handlePinPress(String(num)) }}
            className="aspect-square bg-[#F7F7F5] rounded-full border border-[rgba(26,26,26,0.06)] text-xl font-medium text-[#1A1A1A] hover:bg-[#ebebeb] active:scale-95 transition-all duration-150 shadow-sm"
          >
            {num}
          </button>
        ))}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div />
        <button
          onClick={() => { triggerHaptic(); handlePinPress('0') }}
          className="aspect-square bg-[#F7F7F5] rounded-full border border-[rgba(26,26,26,0.06)] text-xl font-medium text-[#1A1A1A] hover:bg-[#ebebeb] active:scale-95 transition-all duration-150 shadow-sm"
        >
          0
        </button>
        <button
          onClick={() => { triggerHaptic(); setPin(p => p.slice(0, -1)) }}
          className="aspect-square bg-[#F7F7F5] rounded-full border border-[rgba(26,26,26,0.06)] text-[#666] hover:bg-[#ebebeb] active:scale-95 transition-all duration-150 shadow-sm flex items-center justify-center"
        >
          <Delete size={18} />
        </button>
      </div>
